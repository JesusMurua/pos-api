...
- **Enterprise ($999):** Multi-Sucursal (Franquicias), API Pública, **Recipe & Waste Inventory** (control de recetas y mermas).

### 1.2 Quick Service (Cafeterías y Fast Food)
*Flujo rápido, sin gestión de mesas.*
- **Free ($0):** 1 Caja, Hardware Core, 50 productos.
- **Basic ($149) [RECOMMENDED]:** Productos ilimitados, Facturación CFDI, Realtime KDS de Barra. **[OVERRIDE]** Este plan habilita `RealtimeKds` vía `PlanBusinessTypeFeatureOverride`, saltando la regla global que lo restringe a Pro+.
...
- **Pro ($349):** **Multi-Warehouse Inventory** (inventario multi-bodega), Reportes Comparativos, Alertas de Stock, **Multi-Till (Ilimitado)**.
*(Enterprise plan NO APLICA)*
...
4. **3D Matrix Resolution:** El gating se evalúa usando `PlanFeatureMatrix` + `BusinessTypeFeature`. Si existe una regla específica para un cruce de Plan y Giro, se debe usar una tabla de `PlanBusinessTypeFeatureOverride` cuyo `IsEnabled` tiene precedencia sobre la lógica 2D (ej. Quick Service Basic × `RealtimeKds`).
